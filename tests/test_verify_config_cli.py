...
def _make_cmds_dir(tmp_path):
    ...
    # Mirror the real tetracorder6.00a.cmds layout: DATASETS/ and restart_files/
    # are SIBLINGS under the cmds root (restart_files is NOT nested in DATASETS).
    (d / "DATASETS").mkdir(parents=True)
    (d / "restart_files").mkdir(parents=True)
    (d / "DELETED.channels").mkdir(parents=True)
    (d / "DATASETS" / "emit_c").write_text("data= EMITc\nrestart= r1-emitc\n")
    shutil.copy(FIXTURES / "r1-emitc", d / "restart_files" / "r1-emitc")
